...
// Params parses the params and returns the value of type T. The params can be an array or an object. If the params are
// an array, the fields of T must be in the same order as the elements of the array. If the params are an object, the
// fields of T must have the same name as the keys of the object or have a json tag with the same name. Only exported
...
func ParamsType(t reflect.Type, params []byte) (any, error) {
	...
	if t.Kind() != reflect.Struct &&
		...
		return reflect.Zero(t).
				Interface(),
			fmt.Errorf(
				"params must be a struct or a pointer: %w",
				ErrInvalidParams,
			)
	}
...
func (pw *paramsWrapper) parsePositional(b []byte) error {
	...
	_, err := dec.Token()
	if err != nil {
		return fmt.Errorf("error decoding array: %w", err)
	}
	...
	for i := range structT.NumField() {
		...
		if err := dec.Decode(field.Interface()); err != nil {
			return fmt.Errorf(
				"error decoding field %q: %w: %w",
				structT.Field(i).Name,
				ErrInvalidParams,
				err,
			)
		}
